...
.data
    ...
    number      db  169d    ;stores the random value
 
    ;add LineBreak to strings
    CR          equ 13d
    LF          equ 10d
 
    ;String messages
    prompt      db  CR, LF,'enter a valid number : $'
    lessMsg     db  CR, LF,'Value if Less than u entered ','$'
    moreMsg     db  CR, LF,'Value is More than u entered', '$'
    equalMsg    db  CR, LF,'You have made fine Guess!', '$'
    overflowMsg db  CR, LF,'Error - Number out of range!', '$'
    retry       db  CR, LF,'Retry [y/n] ? ' ,'$'
 
    guess       db  0d      ;store value user entered
    errorChk    db  0d      ;value is in range?
